...
// ── Design tokens (matches HelpOverlay) ───────────────────────────────
const OVERLAY_BG: Color = Color::Rgb(10, 10, 10);
const HEADER_FG: Color = Color::Rgb(176, 196, 200);
const SECTION_FG: Color = Color::Rgb(130, 130, 130);
const CURSOR_FG: Color = Color::Rgb(176, 196, 200);   // ► selected row
const LABEL_FG: Color = Color::Rgb(190, 190, 190);
const VALUE_FG: Color = Color::Rgb(176, 196, 200);    // numeric / enum value
const CHECK_ON: Color = Color::Rgb(130, 200, 150);    // [x] checked
const CHECK_OFF: Color = Color::Rgb(70, 70, 70);      // [ ] unchecked
...
impl<'a> Widget for SettingsOverlay<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        render_full(self.state, area, buf);
    }
}
...
fn item_row(buf: &mut Buffer, y: u16, left: u16, right: u16, cursor: usize, idx: usize, row: ItemRow<'_>) -> u16 {
    let is_selected = cursor == idx;

    // Arrow or space prefix.
    let prefix = if is_selected { "\u{25ba} " } else { "  " }; // ► or spaces
    let prefix_style = Style::default()
        .fg(if is_selected { CURSOR_FG } else { OVERLAY_BG })
        .bg(OVERLAY_BG)
    ...
    if let Some(val) = row.value_str {
        ...
        x = put(buf, x, y, val, Style::default().fg(VALUE_FG).bg(OVERLAY_BG).add_modifier(Modifier::BOLD), right);
    }
...
fn render_full(state: &EditorState, area: Rect, buf: &mut Buffer) {
    ...
    y = item_row(buf, y, margin, right, cur, 0, ItemRow { label: "Line numbers",          hint: Some("Ctrl+L"),            checked: Some(cfg.editor.line_numbers),    value_str: None });
    y = item_row(buf, y, margin, right, cur, 1, ItemRow { label: "Relative numbers",      hint: None,                      checked: Some(cfg.editor.relative_numbers), value_str: None });
    y = item_row(buf, y, margin, right, cur, 2, ItemRow { label: "Syntax highlighting",   hint: None,                      checked: Some(cfg.editor.syntax_highlight), value_str: None });
    y = item_row(buf, y, margin, right, cur, 3, ItemRow { label: "Auto-indent on Enter",  hint: None,                      checked: Some(cfg.editor.auto_indent),      value_str: None });
    y = item_row(buf, y, margin, right, cur, 4, ItemRow { label: "Expand tabs to spaces", hint: None,                      checked: Some(cfg.editor.expand_tabs),      value_str: None });
    y = item_row(buf, y, margin, right, cur, 5, ItemRow { label: "Tab width",             hint: Some("\u{2190} \u{2192}"), checked: None,                              value_str: Some(&tab_w_str) });
    y = item_row(buf, y, margin, right, cur, 6, ItemRow { label: "Indent guides",         hint: None,                      checked: Some(cfg.editor.indent_guides),    value_str: None });
    y = item_row(buf, y, margin, right, cur, 7, ItemRow { label: "Indent error hints",    hint: None,                      checked: Some(cfg.editor.indent_errors),    value_str: None });
    y += 1;
    ...
    if y + 3 < area.bottom() {
        ...
        y += 1;
        y = item_row(buf, y, margin, right, cur, 8, ItemRow { label: "Strategy", hint: Some("\u{2190} \u{2192}"), checked: None, value_str: Some(&cfg.clipboard.strategy) });
        y += 1;
    }
...
fn render_compact(area: Rect, buf: &mut Buffer) {
    ...
    let style = Style::default().fg(HEADER_FG).bg(OVERLAY_BG);
    let mut cx = x;
    for ch in msg.chars() {
